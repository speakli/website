...
const DotGrid = ({
  ...
  useEffect(() => {
    ...
    const resizeHandler = buildGrid as EventListener;
    ...
    return () => { if (ro) ro.disconnect(); else window.removeEventListener('resize', resizeHandler); };
  }, [buildGrid]);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (speed > maxSpeed) { const scale = maxSpeed / speed; vx *= scale; vy *= scale; speed = maxSpeed; }
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity && !dot._inertiaApplied) {
          ...
          gsap.to(dot, {
            xOffset: pushX, yOffset: pushY, duration: 0.2, ease: 'power2.out',
            onComplete: () => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (dist < shockRadius && !dot._inertiaApplied) {
          ...
          gsap.to(dot, {
            xOffset: pushX, yOffset: pushY, duration: 0.15, ease: 'power3.out',
            onComplete: () => {
              gsap.to(dot, { xOffset: 0, yOffset: 0, duration: returnDuration, ease: 'elastic.out(1, 0.75)' });
              dot._inertiaApplied = false;
            }
    ...
    const throttledMove = throttle(onMove as (...args: unknown[]) => void, 16);
